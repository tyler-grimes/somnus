...
const apiRoute = (handler: (req: Request, res: Response) => Promise<unknown>) =>
...
app.get('/api/episodes', apiRoute(async (_req, res) => {
...
app.get('/api/cc-sessions', apiRoute(async (_req, res) => {
  const { rows } = await pool.query(`
...
app.get('/api/memory', apiRoute(async (_req, res) => {
...
app.get('/api/spend', apiRoute(async (_req, res) => {
...
app.get('/api/graph', apiRoute(async (_req, res) => {
...
app.get('/api/page/:id', apiRoute(async (req, res) => {
  const { id } = req.params;
...
app.post('/api/chat', apiRoute(async (req, res) => {
...
app.get('/api/chat/history', apiRoute(async (_req, res) => {
  const { rows } = await pool.query(
...
app.get('/api/chat/:id', apiRoute(async (req, res) => {
  const { id } = req.params;
  if (!PAGE_ID_RE.test(id)) return res.status(400).json({ error: 'invalid id' });
  const { rows } = await pool.query(
